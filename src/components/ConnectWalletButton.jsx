    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <button 
            onClick={closeModal} 
            className="text-slate-400 hover:text-slate-100 p-1 rounded-full hover:bg-slate-700/60 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-800"
        ...
        {wallets.length === 0 && (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-slate-400">
              Please install an Aptos-compatible wallet.
            </p>
          </div>
        )}
        ...
        <div className="space-y-2.5 max-h-60 sm:max-h-80 overflow-y-auto pr-1 custom-scrollbar"> 
          {wallets.map((w) => (
            <button
              ...
              // Enhanced glassmorphism for wallet list items
              className="w-full flex items-center text-left px-4 py-3.5 rounded-xl 
                         bg-white/5 backdrop-blur-md border border-white/10 
                         hover:bg-white/10 hover:border-white/20
                         dark:bg-slate-700/30 dark:border-slate-600/50 
                         dark:hover:bg-slate-700/50 dark:hover:border-slate-500/60
                         ...
                         group transform hover:scale-[1.02] hover:shadow-lg active:scale-95"
            ...
            >
              {w.icon && (
                <img 
                  src={w.icon} 
                  alt={`${w.name} icon`} 
                  className="w-8 h-8 mr-4 rounded-full object-contain shadow-md flex-shrink-0" 
                />
              )}
