// File: src/components/StakeUnstakeControls.jsx
...
export default function StakeUnstakeControls({ account, onTransactionSuccess, userStake, connected, walletBalance = 0 }) {
    ...
    const handleTransaction = async (functionName, amountOctas) => {
        ...
        try {
            ...
            setTxResult({ message: `Transaction successful! Hash: ${pendingTx.hash.substring(0, 10)}...` });
            if (onTransactionSuccess) setTimeout(() => onTransactionSuccess(), 1500); 
            setAmountApt('');
        } catch (error) {
            let errorMessage = error.message || String(error);
            if (errorMessage.includes('User has rejected')) errorMessage = 'Transaction rejected by user.'; 
